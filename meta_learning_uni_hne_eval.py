...
def do_final_learning(model, x_ftrain, ystatus_ftrain, y_ftrain, lr_inner, n_inner, shots_n, n_iters, reg_scale):
    ...
    for nn in range(n_iters):
        start = time.time()
        ind = random.sample(range(x_ftrain.shape[0]), shots_n)
...
def plot_survival_curves(times, events, scores):
...
if __name__ == '__main__':
    ...
    scores = np.mean(score_test_list, axis=0)
    plot_survival_curves(y_test, ystatus_test, scores)
