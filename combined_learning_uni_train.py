...
def train(y_holdout, x_holdout, ystatus_holdout, y_train, x_train, ystatus_train, checkpoint):
    ...
    x = tf.compat.v1.placeholder(tf.float32, [None, FEATURE_SIZE], name='input_data')
    ...
    dense_layer1 = tf.keras.layers.Dense(units=6000, activation=tf.nn.relu, kernel_regularizer=regularizer)(x)
    dense_drop1 = tf.nn.dropout(dense_layer1, rate=1-keep_prob)
    dense_layer2 = tf.keras.layers.Dense(units=2000, activation=tf.nn.relu, kernel_regularizer=regularizer)(dense_drop1)
    dense_drop2 = tf.nn.dropout(dense_layer2, rate=1-keep_prob)
    dense_layer3 = tf.keras.layers.Dense(units=200, activation=tf.nn.relu, kernel_regularizer=regularizer)(dense_drop2)
