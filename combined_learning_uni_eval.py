...
def predict(FEATURE_SIZE, ROOT_PATH, CHECKPOINT_NAME, x_test, y_test, ystatus_test, x_train, y_train, ystatus_train):
    ...
    dense_layer1 = tf.keras.layers.Dense(units=6000, activation=tf.nn.relu)(x)
    dense_drop1 = tf.nn.dropout(dense_layer1, rate=1-keep_prob)
    dense_layer2 = tf.keras.layers.Dense(units=2000, activation=tf.nn.relu)(dense_drop1)
    dense_drop2 = tf.nn.dropout(dense_layer2, rate=1-keep_prob)
    dense_layer3 = tf.keras.layers.Dense(units=200, activation=tf.nn.relu)(dense_drop2)
...
if __name__ == '__main__':
    ...
    REG_SCALE = 0.1  # Assuming you use the same regularization scale as in training
